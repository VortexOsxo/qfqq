...
class EmailValidationDataHandler:
    @classmethod
    def create_request(cls, email: str, code: str, time: str) -> bool:
        try:
            with get_db_access() as conn:
                cur = conn.cursor()
                cur.execute("DELETE FROM public.emailVerificationRequests WHERE email = %s", (email,))

                query = "INSERT INTO public.emailVerificationRequests (email, code, date) values (%s, %s, %s);"
    ...
    def get_date(cls, email: str, code: str) -> str | None:
        query = "SELECT date FROM public.emailVerificationRequests WHERE email = %s AND code = %s;"
        params = (email, code)
